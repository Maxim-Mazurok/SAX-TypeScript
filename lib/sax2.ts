...
export class SAX {
    ...
    private parser: (strict, opt) => SAXParser;
    ...
    private cdata;
    private script;
    private startTagPosition: number;
    ...
    private static isQuote(c) {
        return c === '"' || c === '\'';
    }

    private static isAttribEnd(c) {
        return c === '>' || SAX.isWhitespace(c);
    }

    private static isMatch(regex, c) {
        return regex.test(c);
    }

    private static notMatch(regex, c) {
            ...
            if (len > maxAllowed) {
                ...
                switch (this.BUFFERS[i]) {
                    ...
                    case 'script':
                        ...
                        break;
                    default:
                        this.error('Max buffer length exceeded: ' + this.BUFFERS[i]);
                }
            }
